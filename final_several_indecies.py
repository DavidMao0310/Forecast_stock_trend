...
def get_variables(ret):
    ...
    '''
    df = pd.DataFrame()
    df['lag1'] = ret['NASDAQ Composite'].shift(1)
    df['lag2'] = ret['NASDAQ Composite'].shift(2)
    for i in corrlist:
        df[str(i)] = ret[str(i)].shift(1)
    df['direction'] = np.where(ret['NASDAQ Composite'].values > 0, 1, -1)
    df = df.dropna()
    return df
